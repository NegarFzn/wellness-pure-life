...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ---------------------------
    // POST: immediate send (from Next button)
    // ---------------------------
    if (req.method === "POST") {
      ...
      const lastSent = userDoc?.challenge_21_mindfulness?.lastEmailSentDay ?? 0;
      if (lastSent >= dayNumber) {
        return res.status(200).json({ message: "Already sent for this day." });
      }
    ...
    for (const user of users) {
      ...
      if (dayNumber < 1 || dayNumber > 21 || lastEmailSentDay >= dayNumber)
        continue;
    ...
    return res
      .status(200)
      .json({ message: `✅ Emails sent to ${totalSent} user(s).` });
  } catch (error) {
