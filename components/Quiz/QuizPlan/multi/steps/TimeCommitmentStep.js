  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      try {
        ...
        if (fitnessQuiz?.questions) {
          const q = fitnessQuiz.questions.find(
            (q) => q.key === "timeCommitment"
          );
          if (q) setQuestionData(q);
        }
      } catch (err) {
        console.error("❌ Failed to fetch timeCommitment question:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className={classes.stepContainer}>
      ...
      <p className={classes.subheading}>
        Choose the amount of time you’re willing to spend on fitness each day.
      </p>
